...
description: "Audit Alien::TinyCDB before a release — cpanfile deps declared and pinned to released Alien::Base/Alien::Base::ModuleBuild, $VERSION present, Changes current, dist.ini's alien_* build config intact, dzil build and test green. Reports blockers; does not fix and never releases."
model: sonnet
...
- **No alienfile here.** This is the `Alien::Base::ModuleBuild` path — the build config is
  the `alien_*` keys in `dist.ini`. Do not report a "missing alienfile"; there is meant
  to be none. (Skill `alien-tinycdb-core` has the mechanism.)

## Checklist

1. **`cpanfile`** — `Alien::Base` as a runtime require; `Alien::Base::ModuleBuild` under
   `configure`; test-only modules under `on 'test'`.
2. **`$VERSION`** — `lib/Alien/TinyCDB.pm` carries one (the bundle supplies it; confirm it
   is not hand-broken).
3. **`dist.ini`** — `[@Author::GETTY]` with the `alien_*` build config intact
   (`alien_repo`, the `alien_pattern_*` trio, `alien_build_command`/`alien_install_command`,
   `alien_autoconf_with_pic = 0`), plus `copyright_year`, author and license.
...
5. **`dzil build`** — clean, no warnings; the generated `Build.PL` and the `alien_*`
   config make it into the tarball.
6. **`dzil test`** — green. The share build needs network, a C compiler and `make`; report
   any skip as a skip, not a pass.
